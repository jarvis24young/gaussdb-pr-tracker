...
7. 优先匹配上游新增 / 删除的精确代码行，判断修复行是否已存在或旧逻辑是否仍存在。
8. 基于 patch 函数名和标识符抽取本地函数级上下文，避免大文件按行号截断导致关键实现缺失。
9. 调用 AI 生成结构化分析。
10. 展示同步状态、风险等级、本地证据和建议。

测试文件过滤采用保守规则：只过滤路径目录明确属于 `test/`、`tests/`、`docs/`、`examples/`、`ci/` 的文件，或文件名明确为 `test_xxx`、`xxx_test`、`xxx.spec` 这类测试命名。不会因为普通源码文件名中包含 `spec`、`test` 等子串就过滤，例如 `dlg_specific.c` 必须作为 ODBC 产品源码参与分析。
...
| Source Context Extractor | 基于 patch 函数名、callee 和关键标识符抽取本地函数级上下文。 |
| Source Filter | 过滤明确测试/文档/示例文件，同时保留产品源码中包含 `spec` 等普通子串的文件。 |
| Rule-Based PreClassifier | 基于精确新增/删除代码行命中结果先给出确定性初判。 |
...
点击“分析”时，后端会自动把内置的 `SOURCE_CONFIRMED_DRIVER_SYNC` 分析方法注入 Prompt。该方法沉淀自数据库客户端调用链分析经验，不依赖 Codex 本机 skill 运行时，因此在内网 MiniMax / Anthropic 兼容接口下也能稳定使用。

该流程要求模型按固定顺序分析：

1. 先识别上游 PR 的真实修复点，包括新增防护逻辑、删除旧逻辑、改动函数和直接 callee。
...
为降低纯模型判断的不稳定性，后端会先构造结构化证据包：

- `addedAnchors`：上游新增的关键修复代码行。
- `removedAnchors`：上游删除的旧逻辑代码行。
- `exactAddedMatches`：本地精确命中的新增修复行。
- `exactRemovedMatches`：本地精确命中的删除旧逻辑。
- `functionNamePresence`：上游涉及函数在本地是否存在。
- `preClassification`：规则层初判结果。

规则初判优先级高于模型自由判断。例如 PR #145 中，如果用户当前本地仓库已经存在：
...
这一步解决了大文件分析中的一个典型问题：如果简单按行号排序再截断，像 `dlg_specific.c` 这类文件可能只截到前部声明，而截不到真正的 `decode()`、`conv_from_hex()` 或修复校验函数实现。当前实现会优先围绕 patch 中出现的函数名和关键标识符抽取本地函数上下文，降低“模型没有看到关键实现所以误判缺失”的概率。

分析结论始终以当前页面或 `.env` 中配置的本地仓库路径为准。不同开发机上的 ODBC/JDBC 仓库可能处于不同分支或补丁状态，因此工具不会假设其他机器上的代码状态。
...
- 大型 PR 的上下文仍可能被截断，但当前已从简单行号截断优化为函数级上下文抽取。
- 内网模型质量会影响分析稳定性。
