...
面向数据库驱动质量排查的 AI 辅助分析工具：自动拉取上游驱动已合入 PR，点击 PR 后分析本地 GaussDB ODBC / JDBC / libpq 代码是否存在相同或相似问题，并输出风险等级、命中文件、本地证据和修复建议。

## 这个项目适合解决什么问题

GaussDB ODBC / JDBC / libpq 等驱动与上游 PostgreSQL 生态代码高度相似。上游已合入的 bug fix PR 往往是非常高价值的质量线索。本工具把“人工浏览上游 PR、找本地相似代码、判断是否需要同步修复”做成可视化工作流，适合用于：
...
- 支持 ODBC / JDBC / libpq 三种驱动 Profile
- 拉取对应上游仓库已合入 PR
...
| libpq | `postgres/postgres` 的 `src/interfaces/libpq/` | `GAUSSDB_LIBPQ_PATH` | 分析 PostgreSQL libpq 目录相关变更对 GaussDB libpq 的影响。 |

libpq Profile 会在分析阶段只关注上游 `src/interfaces/libpq/` 下的 C/H 文件。本地 openGauss libpq 常见路径为：

```text
D:/GaussDB/openGauss-server/src/common/interfaces/libpq
```

如果本地设置的是 `GAUSSDB_SERVER_PATH`，后端会自动推导 `src/common/interfaces/libpq` 路径。需要注意，PostgreSQL 官方开发主流程以邮件列表和提交为主，GitHub PR 数据不一定覆盖全部 libpq 上游修复；当前工具的 libpq Profile 仍以 GitHub merged PR 为追踪入口。
...
GAUSSDB_LIBPQ_PATH=D:/GaussDB/openGauss-server/src/common/interfaces/libpq
```
...
如果不点击导入，后端在未配置本项目 AI Key 时，也会优先把 ClaudeCode 配置作为默认 AI 配置读取。因此内网机器只要已有 ClaudeCode 配置，通常只需要配置当前驱动 Profile 对应的本地路径，例如 `GAUSSDB_ODBC_PATH`、`GAUSSDB_JDBC_PATH` 或 `GAUSSDB_LIBPQ_PATH`。
